...
Поддержка FedProx (проксимальный штраф) и DP (клиппинг + шум).
"""
import os
...
class RecClient(fl.client.NumPyClient):
    ...
    def __init__(self, client_id, model, train_loader, val_loader,
                 ...
                 data_dir="data/processed", mu=0.0, dp_config=None):
        ...
        self._global_params = {}
        self._received_numpy_params = None
    ...
    def _compute_proximal_term(self):
        """FedProx: ||theta_shared - theta_global||^2"""
    ...
    def fit(self, parameters, config):
        ...
        # Сохраняем полученные параметры для DP
        if self.dp_config.get("enabled"):
            self._received_numpy_params = [p.copy() for p in parameters]

        # Snapshot глобальных параметров для FedProx
        if self.mu > 0:
            shared_keys = set(self.model.shared_param_keys())
            self._global_params = {
        ...
        for _ in range(epochs):
            ...
            for batch in self.train_loader:
                ...
                if self.mu > 0:
                    loss = loss + (self.mu / 2.0) * self._compute_proximal_term()
        ...
        shared_params = self.get_parameters()

        # DP: клиппинг и шум перед отправкой на сервер
        if self.dp_config.get("enabled") and self._received_numpy_params is not None:
            shared_params = clip_and_add_noise(
                shared_params,
                self._received_numpy_params,
                self.dp_config["max_grad_norm"],
                self.dp_config["noise_multiplier"],
            )

        return shared_params, n_samples, {"loss": avg_loss}
